...
int main(int argc, char *argv[]) {
    ...
    int min = min_temp_cba(array);
    printf("\nThe minimum temp in Cordoba since 1980 is: %d \n", min);

    /*calculate and show the maimum temperatures of each year*/
    int output[YEARS];
    max_temp_anual(array, output);

    /* calculate and show the maximum temperature for each year*/
    for (unsigned int year = 0; year < YEARS; year++){
        printf("\n The maximum temp in %u is %d \n", year+1980, output[year]);
    }
    
    /*prints the rainiest month per year*/
    month_t rainiest_month[YEARS];
    mes_precip(array, rainiest_month);
    print_month_per_year(array, rainiest_month);
