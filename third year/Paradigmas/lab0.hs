...
-- a)
gralFilterInt :: [Int] -> (Int -> Bool) -> [Int]
gralFilterInt [] g = []
gralFilterInt (x:xs) g | g x == True = x : gralFilterInt xs g
                       | g x == False = gralFilterInt xs g     

--b)
polFilterInt :: [a] -> (a -> Bool) -> [a]
polFilterInt [] g = []
polFilterInt (x:xs) g | g x == True = x : polFilterInt xs g
                       | g x == False = polFilterInt xs g  

--c)
soloPares1 :: Int -> Bool
soloPares1 x | mod x 2 == 0 = True
             | mod x 2 /= 0 = False
...
-- a)                 
foldgral :: [Int] -> (Int -> Int ->Int) -> Int
foldgral [] g = 0
foldgral (x:xs) g = g x (foldgral xs g)

--b)
foldpol :: Num a => [a] -> (a -> a -> a) -> a
foldpol [] g = 0
foldpol (x:xs) g = g x (foldpol xs g) 
...
evaluar :: Expr -> Int
evaluar (Suma c b) = (evaluar c) + (evaluar b)
evaluar (Resta c b) = (evaluar c) - (evaluar b)
evaluar (Producto c b) = (evaluar c) * (evaluar b)
evaluar (Division c b) = div (evaluar c)  (evaluar b)
evaluar (Int a) = a
